...
func TestExecuteConnectedUsersCommand(t *testing.T) {
	p := newTestPlugin(t)
	mockAPI := &plugintest.API{}
	...
	for _, testCase := range []struct {
	...
	}{
		{
			...
			setupAPI: func(api *plugintest.API) {
				...
				api.On("SendEphemeralPost", testutils.GetUserID(), &model.Post{
					UserId:    "bot-user-id",
					ChannelId: testutils.GetChannelID(),
					Message:   "| Mattermost User ID | MS Teams User ID | \n|:-------|:-----------|\n|sfmq19kpztg5iy47ebe51hb31w|rioegneonqimomsoqwiew3qeo|",
				}).Return(testutils.GetPost(testutils.GetChannelID(), testutils.GetUserID())).Once()
			},
			setupStore: func(s *mockStore.Store) {
				s.On("ListConnectedUsers").Return(testutils.GetConnectedUsers(1), nil).Times(1)
			},
		},
		{
			description: "User is not a system admin",
		...
		{
			description: "Error in getting connected users",
			setupAPI: func(api *plugintest.API) {
				api.On("HasPermissionTo", testutils.GetUserID(), model.PermissionManageSystem).Return(true).Once()
				api.On("LogDebug", "Unable to get connected users from store", "Error", "error in getting connected users").Once()
				api.On("SendEphemeralPost", testutils.GetUserID(), &model.Post{
					UserId:    "bot-user-id",
					ChannelId: testutils.GetChannelID(),
					Message:   "Something went wrong.",
			...
			setupStore: func(s *mockStore.Store) {
				s.On("ListConnectedUsers").Return(nil, errors.New("error in getting connected users")).Times(1)
			},
		},
		{
			description: "No connected user present",
			setupAPI: func(api *plugintest.API) {
				api.On("HasPermissionTo", testutils.GetUserID(), model.PermissionManageSystem).Return(true).Once()
				api.On("SendEphemeralPost", testutils.GetUserID(), &model.Post{
					UserId:    "bot-user-id",
					ChannelId: testutils.GetChannelID(),
					Message:   "No connected user present.",
				}).Return(testutils.GetPost(testutils.GetChannelID(), testutils.GetUserID())).Once()
			},
			setupStore: func(s *mockStore.Store) {
				s.On("ListConnectedUsers").Return(nil, nil).Times(1)
			},
		},
	} {
		t.Run(testCase.description, func(t *testing.T) {
			...
			testCase.setupStore(p.store.(*mockStore.Store))
			_, _ = p.executeShowConnectedUsersCommand(&model.CommandArgs{
				UserId:    testutils.GetUserID(),
				ChannelId: testutils.GetChannelID(),
			})
			time.Sleep(1 * time.Second)
