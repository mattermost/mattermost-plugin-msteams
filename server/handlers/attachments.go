	...
	for _, a := range msg.Attachments {
		...
		if strings.Contains(a.ContentURL, hostedContentsStr) && strings.HasSuffix(a.ContentURL, "$value") {
			...
			if err != nil {
				...
				ah.plugin.GetMetrics().ObserveFileCount(m.ActionCreated, m.ActionSourceMSTeams, discardedReasonUnableToGetTeamsData, isDirectMessage)
				continue
			}
		} else {
			...
			if fileSize > fileSizeAllowed {
				...
				errorFound = true
				ah.plugin.GetMetrics().ObserveFileCount(m.ActionCreated, m.ActionSourceMSTeams, discardedReasonMaxFileSizeExceeded, isDirectMessage)
				continue
			}
		...
		if fileInfoID == "" {
			ah.plugin.GetMetrics().ObserveFileCount(m.ActionCreated, m.ActionSourceMSTeams, discardedReasonEmptyFileID, isDirectMessage)
			continue
		}
		attachments = append(attachments, fileInfoID)
		ah.plugin.GetMetrics().ObserveFileCount(m.ActionCreated, m.ActionSourceMSTeams, "", isDirectMessage)
		...
		if countFileAttachments == maxFileAttachmentsSupported {
			...
			ah.plugin.GetMetrics().ObserveFilesCount(m.ActionCreated, m.ActionSourceMSTeams, discardedReasonFileLimitReached, isDirectMessage, int64(fileAttachmentsDiscarded))
			break
		}
	}
