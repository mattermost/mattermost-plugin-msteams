...
func (s *SQLStore) SetUserInfo(userID string, msTeamsUserID string, token *oauth2.Token) error {
	...
	if _, err := s.getMasterQueryBuilder().Insert(usersTableName).Columns("mmUserID, msTeamsUserID, token").Values(userID, msTeamsUserID, encryptedToken).Suffix("ON CONFLICT (mmUserID, msTeamsUserID) DO UPDATE SET token = EXCLUDED.token").Exec(); err != nil {
		return err
	}
	return nil
}
...
func (s *SQLStore) GetStats() (*storemodels.Stats, error) {
	...
	query = s.getReplicaQueryBuilder().Select("count(id)").From("users").Where(sq.NotEq{"RemoteId": ""}).Where(sq.Like{"Username": "msteams_%"}).Where(sq.Eq{"DeleteAt": 0})
	row = query.QueryRow()
...
func (s *SQLStore) PrefillWhitelist() error {
	...
	perPage := 100
	for {
		query := s.getReplicaQueryBuilder().Select("mmuserid").From(usersTableName).Where(sq.NotEq{"token": ""}).Offset(uint64(page * perPage)).Limit(uint64(perPage))
		rows, err := query.Query()
		if err != nil {
			return err
		}
...
func (s *SQLStore) GetSizeOfWhitelist() (int, error) {
	query := s.getReplicaQueryBuilder().Select("count(*)").From(whitelistedUsersTableName)
	rows, err := query.Query()
	if err != nil {
		return 0, err
	}
...
func (s *SQLStore) StoreUserInWhitelist(userID string) error {
	query := s.getMasterQueryBuilder().Insert(whitelistedUsersTableName).Columns("mmUserID").Values(userID)
	if _, err := query.Exec(); err != nil {
		if isDuplicate(err) {
...
func (s *SQLStore) IsUserPresentInWhitelist(userID string) (bool, error) {
	query := s.getReplicaQueryBuilder().Select("mmUserID").From(whitelistedUsersTableName).Where(sq.Eq{"mmUserID": userID})
	rows, err := query.Query()
	if err != nil {
		return false, err
	}
