...
func TestRunMSTeamUserIDDedup(t *testing.T) {
	...
	*/
	...
	res, err = store.getQueryBuilder(store.db).Insert(usersTableName).Columns("mmuserid", "msteamsuserid").
	...
	rows, err := store.getQueryBuilder(store.db).Select("mmuserid", "msteamsuserid").From(usersTableName).Query()
	assert.NoError(err)
	var found int
	for rows.Next() {
